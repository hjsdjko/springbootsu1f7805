<template>
	<div class="main-content" :style='{"minHeight":"calc(100vh - 200px)","padding":"10px 0 30px","margin":"0 auto","color":"#333","background":"none","width":"calc(100% - 20px)","fontSize":"14px","height":"100%"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"10px 0px","margin":"0 0 10px","borderColor":"#ccc","overflow":"hidden","borderRadius":"0px","flexWrap":"wrap","background":"#eee","borderWidth":"1px","display":"flex","fontSize":"inherit","borderStyle":"dotted"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"padding":"0 20px 0 0","margin":"0px 0 0px","color":"#333","alignItems":"center","textAlign":"left","display":"flex","justifyContent":"flex-start","borderRadius":"0px","flexWrap":"wrap","background":"none","width":"100%","fontSize":"inherit","order":"1"}'>
					...
					<el-button class="btn18" v-if="isAuth('huodongpingjia','各活动评分数')" type="success" @click="chartDialog1()">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="huodongpingfenChart1" style="width:100%;height:600px;"></div>
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var huodongpingfenChart1 = echarts.init(document.getElementById("huodongpingfenChart1"),'macarons');
        this.$http({
            url: "huodongpingjia/typeStat/huodongpingfen/huodongmingcheng",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    ...
                    yAxis1.push(parseFloat((res[i].一分)));
                    yAxis2.push(parseFloat((res[i].二分)));
                    yAxis3.push(parseFloat((res[i].三分)));
                    yAxis4.push(parseFloat((res[i].四分)));
                    yAxis5.push(parseFloat((res[i].五分)));
                    pArray.push({
                        name: res[i].huodongpingfen
                    })
                }
                var option = {};
                option = {
                ...
                // 使用刚指定的配置项和数据显示图表。
                huodongpingfenChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    huodongpingfenChart1.resize();
                };
            }
        });
      })
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
          ...
          if(this.searchForm.huodongpingfen!='' && this.searchForm.huodongpingfen!=undefined){
            params['huodongpingfen'] = this.searchForm.huodongpingfen
          }
	...
	huodongpingjiastatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'huodongpingjia/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
