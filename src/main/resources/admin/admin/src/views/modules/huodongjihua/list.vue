		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"10px 0px","margin":"0 0 10px","borderColor":"#ccc","overflow":"hidden","borderRadius":"0px","flexWrap":"wrap","background":"#eee","borderWidth":"1px","display":"flex","fontSize":"inherit","borderStyle":"dotted"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"padding":"0 20px 0 0","margin":"0px 0 0px","color":"#333","alignItems":"center","textAlign":"left","display":"flex","justifyContent":"flex-start","borderRadius":"0px","flexWrap":"wrap","background":"none","width":"100%","fontSize":"inherit","order":"1"}'>
					<el-button class="add" v-if="isAuth('huodongjihua','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0px 2px","fontSize":"14px","color":"#333","display":"inline-block"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('huodongjihua','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-button class="btn18" v-if="isAuth('huodongjihua','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
			...
			<div :style='{"padding":"0px","borderColor":"#ccc","margin":"0 0 0px","borderRadius":"0px","background":"#eee","borderWidth":"1px 0px 1px 1px","width":"100%","borderStyle":"solid dotted dotted"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='false' prop="jihuawendang" label="计划文档">
						<template slot-scope="scope">
							<el-button v-if="scope.row.jihuawendang" type="text" size="small" @click="download(scope.row.jihuawendang)">下载</el-button>
                            <span v-else >无</span>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('huodongjihua','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 0 0 4px","fontSize":"14px","color":"#333","height":"auto"}'></span>
								浏览
							</el-button>
							<el-button class="edit" v-if=" isAuth('huodongjihua','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('huodongjihua','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
import AddOrUpdate from "./add-or-update";
		...
		this.$refs["shBatchForm"].validate(valid => {
			if(valid){
				this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
					...
					this.$http({
						url: "huodongjihua/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
						method: "post",
						data: this.batchIds
					}).then(({ data }) => {
						if (data && data.code === 0) {
	...
	huodongjihuastatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'huodongjihua/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
